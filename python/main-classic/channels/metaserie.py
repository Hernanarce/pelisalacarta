...
def mainlist(item):
    ...
    itemlist.append(item.clone(title="Series", action="todas", url="http://metaserie.com/series-agregadas",
                               thumbnail='https://s32.postimg.org/544rx8n51/series.png',
                               fanart='https://s32.postimg.org/544rx8n51/series.png'))

    itemlist.append(item.clone(title="Anime", action="todas", url="http://metaserie.com/animes-agregados",
                               thumbnail='https://s31.postimg.org/lppob54d7/anime.png',
                               fanart='https://s31.postimg.org/lppob54d7/anime.png'))

    itemlist.append(item.clone(title="Buscar", action="search", url="http://www.metaserie.com/?s=",
...
def temporadas(item):
    ...
    for scrapedurl, scrapedtitle in matches:
        ...
        itemlist.append(
            Item(channel=item.channel, action='episodiosxtemp', title=title, fulltitle=item.contentSerieName, url=url,
...
def episodiosxtemp(item):
    ...
    for scrapedurl, scrapedtitle in matches:
        ...
        itemlist.append(Item(channel=item.channel, action="findvideos", title=title, fulltitle=item.fulltitle, url=url,
                             ...
                             contentSeasonNumber=item.contentSeasonNumber, contentEpisodeNumber=contentEpisodeNumber))
    ...
    if texto != '':
        try:
            ...
            for scrapedurl, scrapedtitle in matches:
                ...
                thumbnail = ''
                plot = ''
                itemlist.append(Item(channel=item.channel, action="temporadas", title=title, fulltitle=title, url=url,
                                     thumbnail=thumbnail, plot=plot, folder=True, contentSerieName=title))
...
def findvideos(item):
    ...
    for scrapedid, scrapedurl, scrapedserv in matches:
        ...
        itemlist.append(Item(channel=item.channel, action="play", title=title, fulltitle=item.contentSerieName, url=url,
                             thumbnail=thumbnail, extra=extra, language=IDIOMAS[scrapedid], quality='default',
                             server=server, list_idiomas = list_language, context = autoplay.context))
